...
| **Throughput** | `BatchSize` (applied per call) and reading-order / paragraph grouping via `RecognitionOptions`. `MaxDegreeOfParallelism` caps the recognition stage's threads (default: processor count; `1` = fully sequential) and never changes results. Call `WarmUp(...)` at startup — it runs every model once so the first request is fast. |
| **Hard inputs** | Opt-in `DetectionOptions`: `TileLargeImages` for very long receipts and large drawings (no shrinking to the 4000 px cap), `MinTextHeight = 16` for screenshots, faxes and low-DPI photos, `EnhanceContrast` for faded or unevenly lit scans (detection only — recognition reads the original pixels). |
| **Runtime tuning** | `PaddleOcrServiceOptions.CudnnConvAlgoSearch = CudnnConvolutionAlgorithmSearch.Heuristic` skips cuDNN's per-shape benchmark on NVIDIA GPUs; `AllowIntraOpSpinning = false` stops idle ONNX Runtime threads from spinning on busy servers. |
